...
const ASSETS = ["./", "./index.html", "./manifest.webmanifest", "./icon-180.png", "./icon-192.png", "./icon-512.png", "./icon-512-maskable.png", "./favicon.png", "./pdf.min.js", "./pdf.worker.min.js", "./qr.js", "./jsqr.js", "./zxing.js", "./smart-plan.js", "./clean-plan.js"];
      ...
      Promise.race([
        fetch(e.request.url, { cache: "reload", credentials: "same-origin" }).then(resp => {
          ...
          if (resp && resp.ok) {
            ...
            // [222-E] waitUntil, NOT a bare then. See patch-p222.py.
  ...
  e.respondWith(
    caches.match(e.request, { ignoreSearch: true, cacheName: CACHE }).then(hit =>
      ...
      fetch(e.request).then(resp => {
        ...
        if (resp && resp.ok) {
          const copy = resp.clone();
          e.waitUntil(caches.open(CACHE).then(c => c.put(e.request, copy)));   // [222-E]
        }
        return resp;
